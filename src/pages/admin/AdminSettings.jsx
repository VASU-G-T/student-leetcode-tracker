...
export default function AdminSettings() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-16">
      ...
      <div className="glass-card p-6 sm:p-8 border-amber-500/30 relative overflow-hidden shadow-2xl">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-5 border-b border-slate-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="btn-secondary !py-2 !px-3 text-xs flex items-center gap-1.5 text-slate-300"
        ...
        <div className="my-5 p-4 rounded-xl bg-slate-950/60 border border-slate-800 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <span className="relative flex h-3 w-3">
              ...
              <span className={`relative inline-flex rounded-full h-3 w-3 ${isFirebaseConfigured ? 'bg-emerald-500' : 'bg-amber-500'}`}></span>
            </span>
            <div>
              ...
              <p className="text-[11px] text-slate-400">
                {isFirebaseConfigured 
                  ? `Connected to Firestore (${projectId || 'cloud'}). All edits sync immediately to all browsers.` 
                  : 'Data is saved in this browser. Configure Firebase below to enable real-time sync across other devices and emails.'}
              </p>
            </div>
          </div>

          <span className={`text-[11px] px-2.5 py-1 rounded font-mono font-bold ${isFirebaseConfigured ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'}`}>
            {isFirebaseConfigured ? 'LIVE SYNC ON' : 'LOCAL ONLY'}
          </span>
        </div>

        {testResult && (
          <div className={`p-3 rounded-lg text-xs mb-5 flex items-start gap-2 ${testResult.connected ? 'bg-emerald-500/10 text-emerald-300 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-300 border border-rose-500/20'}`}>
            {testResult.connected ? <CheckCircle2 className="w-4 h-4 shrink-0" /> : <AlertCircle className="w-4 h-4 shrink-0" />}
        ...
        <form onSubmit={handleSaveCloudConfig} className="space-y-4 pt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-[11px] font-semibold uppercase text-slate-400 mb-1">
                Project ID <span className="text-amber-400">*</span>
              </label>
              <input
                type="text"
                value={projectId}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input
                ...
                className="input-field"
              />
              <p className="text-[11px] text-slate-500 mt-1">Default target problem count for all students (standard: 4033).</p>
            </div>
